...
We often include manual steps in our workflow, thinking we will only do something once or twice. However, we invariably end up doing it more often than anticipated. It is very likely that before submitting a paper you will need to prepare a reproducibility package – if not because the journal requires it, because you will want someone outside the project team to do at least a computational reproducibility check. Writing code to re-run years of work will be a daunting task. So do yourself a favor and **think about an automation workflow at the onset of your analytical work**. Setting up a system to automate your workflow and having a process to keep it up-to-date [will save you a lot of time](https://xkcd.com/1205/) over the project’s life-cycle.

Computational reproducibility requires the ability to update and to recreate each research output in your project with a single click. Within a given statistical software, this can be accomplished using a *main script* (you can also have this script run software from another, for example by calling R or Python from Stata and vice-verse). But it is also requires an automated way to get graphs and tables created by that statistical software into your paper or presentation. A common practice that breaks this automation principle, for example, is to export graphs from statistical software, save them as images, and manually load them into a paper or presentation. This practice dangerously increases the likelihood that images will not be updated in the final document when making last minute changes. 

Fortunately, there are many tools that facilitate the automation of the research workflow. One option is to use markdown documents in the same statistical software used for data processing and analysis. Another option that is very helpful if you prefer using LaTeX for document preparation, or use multiple different software in one project, is to create a Makefile. Learning how to use these tools can be time consuming, but remember that not everyone involved in a project needs to know all the nitty-gritty of this setup. The key take-away is to know what you should aim for and where to ask for help when it’s time to put it into practice.

[Quarto: dynamic documents in Python, R, Julia, and Observable](https://quarto.org/)
